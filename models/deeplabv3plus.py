...
# --------本地导入--------
from extension.deeplabv3_module.xception import xception
...
def deeplabv3plus_xception(num_classes, output_stride, pretrained_backbone, separable_conv):
    """
    Args:
        num_classes: 类别数
        output_stride: 输出下采样倍率
        pretrained_backbone: 是否使用预训练的backbone
    Returns:
        网络结构
    """
    if output_stride == 8:
    ...
    backbone = xception(pretrained_backbone, replace_stride_with_dilation=replace_stride_with_dilation)
...
def create_model(param):
    return deeplabv3plus_xception(param["category"],
                                  param["output_stride"],
...
if __name__ == '__main__':
    ...
    result = summary(net,input_size=(3, 512, 512))
    print(result)