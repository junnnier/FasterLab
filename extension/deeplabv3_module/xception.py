...
def xception(pretrained_backbone, num_classes=1000, replace_stride_with_dilation=None):
    model = Xception(num_classes=num_classes, replace_stride_with_dilation=replace_stride_with_dilation)
    # 是否加载预训练模型
    if pretrained_backbone:
        assert num_classes == 1000, "num_classes should be {}, but is {}".format(1000, num_classes)
        model = Xception(num_classes=num_classes, replace_stride_with_dilation=replace_stride_with_dilation)
        model.load_state_dict(torch.load(pretrained_backbone))
